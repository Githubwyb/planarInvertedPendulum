...
void MPU_Get_param(double* gyro, double* acce)
{
    ...
    gyro[1] = (data[0] - gyro_offset[0]) / 131.0;
    gyro[2] = (data[1] - gyro_offset[1]) / 131.0;
    gyro[0] = (data[2] - gyro_offset[2]) / 131.0;

    MPU_Get_Accelerometer(data);
    acce[1] = data[0] / 16384.0 * 9.794 - 0.56;
    acce[2] = data[1] / 16384.0 * 9.794;
    acce[0] = data[2] / 16384.0 * 9.794 + 0.16;
}
